function [no_output_gen_video] = IRCE_gen_3ch_movie_ExtraAnnotations(Dilated_label_forROIs, Ch1_corr_IRM, IRM_LUT, Ch2_Impulse, Impulse_LUT, ...
    Ch3_Response, Response_LUT, Time_stamps_address, Video_Name, ROI_n, ...
    channel_labels, STLN_Tracks, roi_IRM_interface, Stats_curr_ROI, Response_Tracks)
    if nargin == 11
        STLN_Tracks = [];
        no_tracks_flag = 1;
    else
    ...
    %         annotation(gcf,'arrow',[0.79 0.82],[0.4 0.51],'linewidth',2,'Color',[1 1 0]);
        ...
        % Works with multi ROIs in one Image
        % Add the mask for the LysoResponse Above bkgd to the
        [B,~] = bwboundaries(Stats_curr_ROI.mask_response_guass(:,:,i) == 1,'noholes');
        hold on
        for k = 1:length(B)
           boundary = B{k};
           %plot(boundary(:,2)+(size(data,2)/Channels)*0, boundary(:,1),'Color',"#00FFFF",'LineStyle',':','LineWidth',2)
           plot(boundary(:,2)+(size(data,2)/Channels)*1, boundary(:,1),'Color',"#A8F9A9",'LineStyle',':','LineWidth',2) % Channel 2
           plot(boundary(:,2)+(size(data,2)/Channels)*2, boundary(:,1),'Color',"#A8F9A9",'LineStyle',':','LineWidth',2) % Channel 3
        end
        hold off
        ...
        % Reponse Localizations into the 3rd channel
        if i <= size(Response_Tracks,2)
            if ~all(isnan(Response_Tracks(:,i,1)))
                ...
                for k = 1:length(B)
                   boundary = B{k};
                   x_outline = boundary(:,2)+(size(data,2)/Channels)*2;
                   y_outline = boundary(:,1);
                    ...
                    x = x+(size(data,2)/Channels)*2+1;
                    y = y+1;
                    [in,~] = inpolygon(x,y, x_outline, y_outline);
                    scatter(x(in),y(in),85,'o','filled','MarkerEdgeColor',[1 0 0],'LineWidth',2,'MarkerFaceAlpha',0.6,'MarkerEdgeAlpha',0.6)
                end 
            end
        end
        hold off
