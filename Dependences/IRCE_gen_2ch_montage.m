function [no_output_gen_montage] = IRCE_gen_2ch_montage(Dilated_label_forROIs, Ch1_corr_IRM, IRM_LUT, Ch3_Response, Response_LUT, Time_stamps_address, Montage_Name, ROI_n,channel_labels, roi_IRM_interface)
    num_montage_frames = 10;

    %----------------------------------------------------------
    % Check if Ch1, Ch2 needs to be resized to the largest data in t
    %----------------------------------------------------------
    maxT = max([size(Ch1_corr_IRM,3), size(Ch3_Response,3)]);
    ...
    for i = 1:num_montage_frames
        ...
        data(size(Data.IRM,1)+1:size(Data.IRM,1)*2, 1+(size(Data.IRM,2)*(i-1)):(size(Data.IRM,2)*(i))) = ch3_data(:,:,montage_idx(i));
    end
    ...
    try
        ...
        text(offset_from_left,size(Data.IRM,1)*2-offset_from_bot,channel_labels.Ch3,'fontsize',channel_text_font_size,'color',text_color,'rot',0,'FontWeight','bold')
    catch
        text(offset_from_left,size(Data.IRM,1)-offset_from_bot,'Ch3','fontsize',channel_text_font_size,'color',[0 0 0],'rot',0,'FontWeight','bold')
    end
    ...
    line([1 size(data,2)],[size(data,1)/2+1 size(data,1)/2+1],'Color',[1 1 1],'LineWidth',2);
    ...
    for i = 1:num_montage_frames
        ...
        for k = 1:length(B)
           boundary = B{k};
           plot(boundary(:,2)+((i-1)*size(Data.IRM,2)), boundary(:,1)+size(Data.IRM,1),'Color',"#00FFFF",'LineStyle',':','LineWidth',2)
        end   
    end
    hold off
