function [Dilated_label_ROIs,  Base_label_ROIs] = IRCE_ApplyMaskEdits(roi_mask_Manual_Crops, Raw_label_ROIs, Ch1_corr_IRM, Specific_ROIs, Base_label_ROIs)
    if isempty(Specific_ROIs)
        Specific_ROIs = 1:size(Base_label_ROIs,1);
    end
    %---------------------------------------------------------%
    ...
    if ~isempty(roi_mask_Manual_Crops)
        for i = Specific_ROIs
    ...
    for i = Specific_ROIs
        %Base_label_ROIs{i,1} = KLS_Label_previousFrameCentroid(Base_label_ROIs{i,1});
    ...
    %---------------------------------------------------------%
    pixel_size = 0.157;
    min_size = 25; % minimium contact area a cell must have for the entire trace to be included
