function [no_output_gen_montage] = IRCE_gen_2ch_montage_IRM_Binding(Dilated_label_forROIs, Ch1_corr_IRM, IRM_LUT, Ch2_Impulse, Binding_LUT, Time_stamps_address, Montage_Name, ROI_n,channel_labels, roi_IRM_interface)
    num_montage_frames = 10;

    %----------------------------------------------------------
    % Check if Ch1, Ch2 needs to be resized to the largest data in t
    %----------------------------------------------------------
    maxT = max([size(Ch1_corr_IRM,3), size(Ch2_Impulse,3)]);
    ...
    ch3_LUT = Binding_LUT; % Response - LysoBrite Blue
    ...
    try
        ...
        text(offset_from_left,size(Data.IRM,1)-offset_from_bot,'Ch2','fontsize',channel_text_font_size,'color',[0 0 0],'rot',0,'FontWeight','bold')
    end
